...
export function registerPortfolioTools(server: McpServer, client: IBClient): void {
  server.registerTool('get_positions', {
    title: 'Get Positions',
    description: 'Get all open positions for an account. Auto-paginates through all pages. Returns stocks, options, futures, etc. Each position includes: symbol, quantity, avg cost, market value, unrealized P&L, daily P&L, asset class. Option positions also include: strike, right (C/P), expiry, multiplier.',
    inputSchema: {
