...
export function registerPnlTools(server: McpServer, client: IBClient): void {
  ...
  server.registerTool('get_performance', {
    ...
    description: 'Get historical account performance including NAV and time-weighted returns over a configurable period.',
    inputSchema: {
    ...
    try {
      const data = await client.post('/pa/performance', { acctIds: accountIds, freq: period });
      return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }] };
    } catch (error) {
      return { content: [{ type: 'text', text: `Error: ${(error as Error).message}` }], isError: true };
    }
  ...
  server.registerTool('get_transaction_history', {
    ...
    description: 'Get transaction history for accounts with optional filters.',
    inputSchema: {
      accountIds: z.array(z.string()).min(1).describe('Array of account IDs'),
